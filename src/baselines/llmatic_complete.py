"""
LLMatic 完整基线模型

基于LLM+质量多样性的神经架构搜索 - 完整端到端实现
包含真实的LLM驱动的架构搜索
"""

import torch
import torch.nn as nn
import sys
from pathlib import Path
from typing import Dict, Optional, Callable, Any

sys.path.insert(0, str(Path(__file__).parent.parent))

from .base_complete_model import CompleteBaselineModel
from .llmatic import LLMatic, compute_behavior_descriptor


class LLMaticCompleteModel(CompleteBaselineModel):
    """
    LLMatic完整模型

    包含真实的LLM驱动的架构搜索过程
    """

    def __init__(self, input_dims: Dict[str, int], hidden_dim: int = 256,
                 num_classes: int = 10, is_regression: bool = False, **kwargs):
        # 搜索参数
        self.population_size = kwargs.get('population_size', 10)
        self.num_iterations = kwargs.get('num_iterations', 20)
        self.behavior_bins = kwargs.get('behavior_bins', 5)

        # 延迟初始化LLMatic（在搜索时）
        self.llmatic_searcher = None
        self.best_architecture_code = None
        self.best_fusion_module = None

        # 是否使用简化版（不使用LLM，快速测试）
        self.use_simple_version = kwargs.get('use_simple_version', False)

        super().__init__(input_dims, hidden_dim, num_classes, is_regression)

    def _create_fusion_module(self) -> nn.Module:
        """创建融合模块（初始使用简单版本）"""
        # 初始时使用简单融合，搜索后会替换
        from .base_complete_model import SimpleAttentionFusion
        return SimpleAttentionFusion(self.hidden_dim)

    def search_architecture(
        self,
        evaluator: Optional[Callable[[str], Dict[str, float]]] = None,
        api_contract: Optional[Dict] = None,
        verbose: bool = True
    ):
        """
        执行LLMatic架构搜索

        Args:
            evaluator: 评估函数，输入代码，返回指标字典
            api_contract: API契约定义
            verbose: 是否打印进度
        """
        if self.use_simple_version:
            if verbose:
                print("⚠️ Using simple version (no LLM search)")
            return

        # 初始化LLMatic搜索器
        self.llmatic_searcher = LLMatic(
            population_size=self.population_size,
            num_iterations=self.num_iterations,
            api_contract=api_contract,
            behavior_bins=self.behavior_bins
        )

        # 默认评估器（简化版）
        if evaluator is None:
            evaluator = self._default_evaluator

        # 执行搜索
        if verbose:
            print("\n" + "=" * 70)
            print("🔍 LLMatic Architecture Search")
            print("=" * 70)
            print(f"Population size: {self.population_size}")
            print(f"Iterations: {self.num_iterations}")
            print(f"Behavior bins: {self.behavior_bins}")
            print()

        try:
            best_solution = self.llmatic_searcher.search(evaluator, verbose=verbose)

            if best_solution:
                self.best_architecture_code = best_solution.code
                # 实例化最佳架构
                self._instantiate_best_architecture()

                if verbose:
                    print(f"\n✅ LLMatic Search Complete")
                    print(f"   Best fitness: {best_solution.fitness:.4f}")
                    print(f"   Archive size: {len(self.llmatic_searcher.archive.grid)}")

        except Exception as e:
            print(f"⚠️ LLMatic search failed: {e}")
            print("   Falling back to simple architecture")

    def _default_evaluator(self, code: str) -> Dict[str, float]:
        """
        默认评估器（简化版）

        实际使用时应该传入真正的评估函数
        """
        # 简化评估：基于代码复杂度打分
        lines = code.strip().split('\n')
        complexity = len([l for l in lines if 'nn.' in l or 'torch.' in l])

        # 模拟随机准确率（实际应该真正训练和评估）
        import random
        accuracy = 0.3 + random.random() * 0.4  # 0.3 - 0.7
        flops = complexity * 1000000

        return {
            'accuracy': accuracy,
            'flops': flops,
            'fitness': accuracy
        }

    def _instantiate_best_architecture(self):
        """实例化最佳架构代码"""
        if not self.best_architecture_code:
            return

        try:
            namespace = {}
            exec(self.best_architecture_code, namespace)

            # 查找生成的类
            FusionClass = None
            for name, obj in namespace.items():
                if isinstance(obj, type) and issubclass(obj, nn.Module) and name != 'Module':
                    FusionClass = obj
                    break

            if FusionClass:
                self.best_fusion_module = FusionClass()
                # 替换融合模块
                self.fusion = self.best_fusion_module

        except Exception as e:
            print(f"⚠️ Failed to instantiate architecture: {e}")

    def get_search_summary(self) -> Dict[str, Any]:
        """获取搜索摘要"""
        if not self.llmatic_searcher:
            return {"status": "not_searched"}

        return {
            "status": "completed",
            "iterations": self.llmatic_searcher.current_iteration,
            "archive_size": len(self.llmatic_searcher.archive.grid),
            "coverage": self.llmatic_searcher.archive.coverage(),
            "best_fitness": self.llmatic_searcher.archive.get_best().fitness if self.llmatic_searcher.archive.get_best() else 0.0
        }


__all__ = ['LLMaticCompleteModel']
